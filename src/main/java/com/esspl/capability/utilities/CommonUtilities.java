package com.esspl.capability.utilities;
...
import com.esspl.capability.resources.Status;
...
public class CommonUtilities extends Base {
	...
	public void addCucumberStep(String description, String stepName, boolean addScreenshot, Status status)
			throws IOException {

		switch (status) {
		case FAIL:
			if (addScreenshot) {
				String path = takescreenshot(System.getProperty("user.dir") + "//Screenshots//" + stepName + ".png",
						driver);
				// log.fail(description,MediaEntityBuilder.createScreenCaptureFromPath(path).build());
			} else {
				//// logger.log(com.aventstack.extentreports.extentreports.Status.FAIL,
				//// description);
		...
		case PASS:
			if (addScreenshot) {
				...
				Reporter.addStepLog(description);
				System.out.println("in else block");
				//// logger.log(com.aventstack.extentreports.extentreports.Status.PASS,
				//// description);
	...
	public void highlightElement(WebElement element) throws InterruptedException {
		JavascriptExecutor js = (JavascriptExecutor) driver;
		js.executeScript("arguments[0].setAttribute('style', arguments[1]);", element,
				"color: ; border: 15px solid yellow;");
	...
	public WebElement executeStep(WebElement element, String description) throws IOException {
		...
		System.out.println("-----------execute step--------------"+screenshotpath);
		String status;
		try {
...
//		  stepDetails.add(stepDetails);
//		  serielNo++ ; 
//		  if(status.equalsIgnoreCase("Fail") && SuiteRunner.suiteRun == true) { 
...
//			  }
	...
	/*
	 * This method is used for checking expected data with actual data if it fails
	 * then execution ends
	 * 
	 * @CreatedBy - Debasis
	 * 
	...
	public <T> T assertEquals(T Expected, T Actual) {
		try {
			...
			e.getMessage();
			//updateStep("Validtion Failed - Expected: " + Expected + "Actual :" + Actual, "Steps",
			//		"Fail - Exception : " + "\n\t\t" + e.getClass().getSimpleName());
		}
		return Actual;
	}
...
//	}
	...
	/*
	 * This method is used for performing some operation when it is not working
	 * normally
	 * 
	 * @CreatedBy - Debasis
	 * 
	...
	public void executeJScript(String action, WebElement element) {
		try {
			...
			//updateStep("Error in Executing javaScript", "Steps",
			//		"Fail - Exception: " + "\n\t\t" + e.getClass().getSimpleName());
		}
	}

	/*
	 * This method is used for clicking on an element when normal click operation is
	 * not working
	 * 
	 * @CreatedBy - Debasis
	 * 
	...
	/*
	 * This method is used for getting the text of an element when normal GetText is
	 * not working
	 * 
	 * @CreatedBy - Debasis
	 * 
...
//		}
	...
	/*
	 * This method returns true, if title of the webpage matches with the title of
	 * the provided data else false
	 * 
	 * @CreatedBy - Debasis
	 * 
	 * @CreatedDate - 21 Aug 2019
	...
	public Boolean assertTitle(String title) throws IOException, InterruptedException {
		try {
			...
			e.getMessage();
			//updateStep("Error in getting page title", "Steps",
			//		"Fail - Exception: " + "\n\t\t" + e.getClass().getSimpleName());
			return false;
		}
	 ...
	 * 
	 */
	public void selectDropdownByVisibleText(WebElement ele, String Value, String description)
	...
	/*
	 * The method selects the option based on the VALUE attribute value of option
	 * tag
	 * 
	 * @CreatedBy - Debasis
	 * 
	 ...
	 * 
	 */
	public void selectDropdownByVisibleValue(WebElement ele, String Value, String description)
			throws IOException, InterruptedException {
		try {
			highlightElement(ele);
			Select se = new Select(ele);
	...
	/*
	 * The method deselects the option by Zero based INDEX value passed as an
	 * integer parameter
	 * 
	 * @CreatedBy - Debasis
	 * 
	 ...
	 * 
	 */
	public void selectDropdownByIndex(WebElement ele, int index, String description)
			throws IOException, InterruptedException {
		try {
			highlightElement(ele);
			Select se = new Select(ele);
	...
	/*
	 * The method returns true if text of the element equals to the provided text
	 * otherwise fale.
	 * 
	 * @CreatedBy - Debasis
	 * 
	...
	/*
	 * The method returns true if text of the element contains or partially contains
	 * the provided text otherwise fale.
	 * 
	 * @CreatedBy - Debasis
	 * 
	 * @CreatedDate - 21 Aug 2019
	...
	/*
	 * The method Moves the mouse to an offset from the top-left corner of the
	...
	/*
	 * This Method Performs a double-click at the current mouse location using
	 * Actions class.
	 * 
	 * @CreatedBy - Debasis
	 * 
	...
	/*
	 * This Method Performs a context-click at middle of the given element using
	 * Actions class.
	 * 
	 * @CreatedBy - Debasis
	 * 
	...
	/*
	 * This convenience method performs click-and-hold at the location of the source
	 * element, moves to the location of the target element, then releases the
	 * mouse. using Actions class.
	 * 
	 * @CreatedBy - Debasis
	 * 
	 * @CreatedDate - 21 Aug 2019
	...
	/*
	 * This convenience method performs click-and-hold at the location of the source
	 * element, moves by a given offset, then releases the mouse using Actions
	 * class.
	 * 
	 * @CreatedBy - Debasis
	 * 
	...
	/*
	 * This Method returns the value of the given attribute as a String of the Web
	 * element
	 * 
	 * @CreatedBy - Debasis
	 * 
	...
	/*
	 * This Method help in pressing Escape keys (Esc) from keyboard using Ations
	 * class
	 * 
	 * @CreatedBy - Debasis
	 * 
	...
	/*
	 * This Method help in pressing Enter keyword from keyboard, keyboard event
	 * using Ations class
	 * 
	 * @CreatedBy - Debasis
	 * 
	...
	public String getCellData(String path, String sheetName, int rowNum, int colNum) throws IOException {
		...
		if (path.contains(".xlsx")) {
			XSSFWorkbook excelWorkBookXlxs = new XSSFWorkbook(excelFileInputStream);
			XSSFSheet excelWorkSheetXlxs = excelWorkBookXlxs.getSheet(sheetName);
			XSSFCell cell = excelWorkSheetXlxs.getRow(rowNum).getCell(colNum);
			cellData = cell.getStringCellValue();
			excelWorkBookXlxs.close();
		} else if (path.contains(".xls")) {
			HSSFWorkbook excelWorkBookXls = new HSSFWorkbook(excelFileInputStream);
			HSSFSheet excelWorkSheetXls = excelWorkBookXls.getSheet(sheetName);
			HSSFCell cell = excelWorkSheetXls.getRow(rowNum).getCell(colNum);
			cellData = cell.getStringCellValue();
			excelWorkBookXls.close();
		}
		return cellData;
	}
	...
	public String generateRandomString(int upperRange) {
		String generatedString = RandomStringUtils.randomAlphabetic(upperRange);
		return (generatedString);
	}
